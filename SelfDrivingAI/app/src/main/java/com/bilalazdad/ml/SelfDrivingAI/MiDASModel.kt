    ...
    init {
        ...
        if (CompatibilityList().isDelegateSupportedOnThisDevice) {
            try {
                ...
                Logger.logInfo("Using GPU Delegate")
            } catch (e: Exception) {
                Logger.logError("Error initializing GPU delegate, falling back to CPU: ${e.message}")
                interpreterOptions.setNumThreads(NUM_THREADS)
                Logger.logInfo("Using CPU with $NUM_THREADS threads")
            }
        } else {
            interpreterOptions.setNumThreads(NUM_THREADS)
            Logger.logInfo("GPU Delegate not supported, using CPU with $NUM_THREADS threads")
        }
